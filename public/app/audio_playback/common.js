...
var common2 = (function() {
  ...
  function runTests(moduleEl) {
    ...
    // has crashed.
    common2.tester.exitCleanlyIsOK();

    common2.tester.addAsyncTest('loaded', function(test) {
      test.pass();
    });
    ...
    common2.tester.waitFor(moduleEl);
    common2.tester.run();
  }
  ...
  function createNaClModule(name, type, tool, path, width, height, attrs, messageHandler, loadedHandler) {
    ...
    moduleEl.setAttribute('name', 'nacl_module2');
    moduleEl.setAttribute('id', 'nacl_module2');
    ...
    // event fires.
    var listenerDiv = document.getElementById('listener2');
  ...
  function handleError(event) {
    // We can't use common2.naclModule yet because the module has not been
    // loaded.
    var moduleEl = document.getElementById('nacl_module2');
  ...
  function handleCrash(event) {
    if (common2.naclModule.exitStatus == -1) {
      updateStatus('CRASHED');
    } else {
      updateStatus('EXITED [' + common2.naclModule.exitStatus + ']');
    }
    if (typeof window.handleCrash !== 'undefined') {
      window.handleCrash(common2.naclModule.lastError);
    }
  }
  ...
  function moduleDidLoad() {
    common2.naclModule = document.getElementById('nacl_module2');
   ...
   */
  function hideModule() {
    // Setting common2.naclModule.style.display = "None" doesn't work; the
    // module will no longer be able to receive postMessages.
    common2.naclModule.style.height = '0';
  }
  ...
  function removeModule() {
    common2.naclModule.parentNode.removeChild(common2.naclModule);
    common2.naclModule = null;
  }
  ...
  function logMessage(message) {
    ...
    if (e)
      e.textContent = logMessageArray.join('\n');
    console.log(message);
  }
  ...
  function domContentLoaded(name, tool, path, width, height, attrs) {
    ...
    } else if (common2.naclModule == null) {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (body.dataset && body.dataset.name) {
    var loadFunction;
    if (!body.dataset.customLoad) {
      loadFunction = common2.domContentLoaded;
